[CmdletBinding()]
param(
 [string]$ConfigPath='C:\EngineeringMCPChatGPT\config\local.oauth.json',
...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot '..\config\oauth.example.json') -Destination $ConfigPath
Write-Output ('CONFIG_CREATED='+$ConfigPath)
Write-Output ('AUTHZ_ROOT_CREATED='+$AuthzRoot)
Write-Output 'NEXT=Replace every example OAuth value, set the trusted Engineering MCP executable and canonical repository, keep contractVersion engineering-c2c/1, review filesystem permissions, and place an HTTPS reverse proxy in front of the private listener.'
